...
- **⚠️ A long-lived `repo.Repository` handle does not auto-refresh (verified live 2026-09-06):** `Manager` caches one open handle per namespace for the process lifetime. That handle's content index reflects the repo as of when it was opened/last refreshed — it does NOT automatically pick up manifests committed later by another writer (Velero). Symptom observed live: kopia-browser's "latest snapshot" for a namespace froze at a fixed date for days while Velero kept backing up normally; a cold-opened process against the same repo saw the true latest immediately. Fix: call the public `Repository.Refresh(ctx) error` method ("reloads the committed content indexes") on every cache hit in `Manager.open()`. It's cheap — reloads only the index blob list, not the full manifest set — so calling it on every access is fine. See DECISIONS.md 2026-09-06.
